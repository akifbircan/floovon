#!/usr/bin/env node
/**
 * tenants_sayfa_izinleri tablosundan profil-ayarlari kayıtlarını kaldırır
...
async function removeProfilAyarlariFromPagePermissions() {
    return new Promise((resolve, reject) => {
        console.log('🔄 tenants_sayfa_izinleri tablosundan profil-ayarlari kayıtları kaldırılıyor...');
        ...
        db.serialize(() => {
            ...
            db.get(`SELECT COUNT(*) as count FROM tenants_sayfa_izinleri WHERE page_id = 'profil-ayarlari'`, (err, row) => {
                if (err) {
                ...
                db.run(`DELETE FROM tenants_sayfa_izinleri WHERE page_id = 'profil-ayarlari'`, (err) => {
                    if (err) {
