...
// Yeni layout: üstte başlık, altta açıklama, butonlar sağda (İptal | Evet, Sil)
function createToastInteractive({ title, message, confirmText = "Evet", cancelText = "İptal", onConfirm, onCancel, isWarning = false, confirmButtonClass = '' }) {
  
  // Zaten aktif bir interactive toast varsa önce kaldır (wrapper varsa wrapper'ı sil)
  document.querySelectorAll('[data-floovon-interactive-toast]').forEach(function (w) {
    if (w.parentNode) w.parentNode.removeChild(w);
  });
  document.querySelectorAll('.toast.interactive').forEach(function (el) {
    if (el.parentNode) el.parentNode.removeChild(el);
  });
  document.querySelectorAll('.toast-overlay').forEach(function (el) {
    if (el.parentNode) el.parentNode.removeChild(el);
  });
  ...
  if (message.includes('var!') && message.includes('Değişiklikleri')) {
    formattedMessage = message.replace(/var!\s+Değişiklikleri/, 'var!<br>Değişiklikleri');
  }
  ...
  wrapper.setAttribute('data-floovon-interactive-toast', '1');
  wrapper.style.cssText = 'position:fixed;inset:0;z-index:2147483647;pointer-events:none;';
  ...
  function removeInteractiveToastNow() {
    removeInteractiveToast(toast, overlay);
    if (wrapper.parentNode) wrapper.parentNode.removeChild(wrapper);
    if (typeof onCancel === "function") onCancel();
  }
  
  window.closeInteractiveToastIfOpen = function () {
    var w = document.getElementById(wrapperId);
    if (w) {
      removeInteractiveToast(toast, overlay);
      if (w.parentNode) w.parentNode.removeChild(w);
    }
    document.querySelectorAll('[data-floovon-interactive-toast]').forEach(function (el) {
      if (el.parentNode) el.parentNode.removeChild(el);
    });
  };
  ...
  toast.querySelector(".toast-btn-cancel").addEventListener("click", function handlerHayir(e) {
    e.preventDefault();
    e.stopPropagation();
    removeInteractiveToastNow();
  }, true);
  
  // ✅ Overlay'e tıklanınca toast kapanmasın - sadece butonlarla kapatılsın
...
// Kopyalama bildirimi (ürün yazısı / yazı not - kopyalanan metni gösterir)
function showCopyAlert(message, copiedText) {
  const alertMessage = document.getElementById('alertMessage');
  const copiedTextElement = document.getElementById('copiedText');
  const urunyazisiAlert = document.getElementById('urunyazisiAlert');
  if (!alertMessage || !copiedTextElement || !urunyazisiAlert) return;
  alertMessage.textContent = message;
  copiedTextElement.textContent = copiedText;
  urunyazisiAlert.style.display = 'block';
  setTimeout(function () {
    urunyazisiAlert.style.display = 'none';
  }, 3000);
}
...
window.showCopyAlert = showCopyAlert;
